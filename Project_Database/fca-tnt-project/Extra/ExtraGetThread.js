...
if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "json") {
    if (!fs.existsSync(process.cwd() + "/Project_Database/Threads.json")) {
        fs.writeFileSync(process.cwd() + "/Project_Database/Threads.json",JSON.stringify({}));
    }
}
...
exports.createData = function(threadID,threadData) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        ...
        try {
            try {
                var data = require(process.cwd() + "/Project_Database/Threads.json");
            }
            catch (e) {
                var data = {};
                fs.writeFileSync(process.cwd() + "/Project_Database/Threads.json",JSON.stringify(data));
            }
...
exports.deleteAll = function(data) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        ...
        try {
            var data1 = require(process.cwd() + "/Project_Database/Threads.json");
            for (let i of data) {
                delete data1[String(i)];
            }
            fs.writeFileSync(process.cwd() + "/Project_Database/Threads.json",JSON.stringify(data1));
        }
        catch (e) {
            console.log(e);
        }
