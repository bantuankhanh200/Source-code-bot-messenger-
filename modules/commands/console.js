...
const SPAM_THRESHOLD = 100; // Số lượng tin nhắn để xác định spam
const SPAM_TIMEFRAME = 30000; // Thời gian trong ms để kiểm tra spam (3 giây)
const LOG_COOLDOWN = 200000; // Thời gian trong ms để tạm dừng log (20 giây)
let messageCounts = {};
...
module.exports.handleEvent = async function ({ api, event, Users }) {
  const thính = require('./../../Project_Database/datajson/tnt.json');
  var tnt = thính[Math.floor(Math.random() * thính.length)].trim();
  ...
  if (!isSpamming) {
    ...
    console.log(
      chalk.hex("#9933FF")(`\n———————————————————————\n→ Nhóm: ${threadName}`) + " \n" +
      chalk.hex("#FF0066")(`→ Người dùng: ${userName}`) + " \n " +
      chalk.hex("#00EE00")(`→ Nội dung: ${messageContent}`) + "\n" +
      chalk.hex("#CC33FF")(`→ Thời gian: ${moment.tz("Asia/Ho_Chi_Minh").format("LLLL")}\n———————————————————————\n`)
    );
  }
};
